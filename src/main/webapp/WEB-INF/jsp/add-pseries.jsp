    ...
    <script type="text/javascript">
        ...
        $(":input[name='Zname']").blur(function(){
            ...
            $(":input[name='CCname']").blur(function(){
                ...
                val = $.trim(val);
                if(0<val.length&&val.length<=5){
                    $("#twonInfo img").attr('src','images/sign_up3.png');
                    $("#twonInfo input").hide();
                }else{
                    $("#twonInfo img").attr('src','images/sign_up2.png');
                    $("#twonInfo input").val("子子类名字格式有误").css("color","#FF0000");
                }
            }).focus(function(){
                $("#twonInfo").show();
                $("#twonInfo input").show();
                $("#twonInfo img").attr('src','images/sign_up1.png');
                $("#twonInfo input").val("子子类名字长度小于6位").css("color","#8d8a8a");
            })
    })
        function addP() {
            var val = $("#Thone option:selected").attr("name");
//            alert(val)
            if(val.length==0){
                $("#threenInfo").show();
                $("#threenInfo input").show();
                $("#threenInfo img").attr('src','images/sign_up2.png');
                $("#threenInfo input").val("请选择子子类").css("color","#FF0000");
            }else{
                window.location.href="/Jd/add-ppseries?thid="+val;
            }
        }
    function s1(){
            $("#twoInfo").hide();
    ...
    function addM(){
        ...
        var url = "${pageContext.request.contextPath}/addJdMclass";
        ...
        $.getJSON(url,args,function (date) {
            if(date.name=="0"){
                ...
                $("#Zone").append("<option name='"+date.mid+"'>"+date.name+"</option>");
                $("#Ztwo").append("<option name='"+date.mid+"'>"+date.name+"</option>");
                $("#Zthree").append("<option name='"+date.mid+"'>"+date.name+"</option>");
//                alert(date.name)
//                alert(date.mid)
                $("#oneInfo").show();
                $("#oneInfo input").show();
                $("#oneInfo img").attr('src','images/sign_up3.png');
                $("#oneInfo input").val("增加成功").css("color","#1db31d");
            }
        })
        }else{
            $("#oneInfo").show();
            ...
            $("#oneInfo input").show();
            $("#oneInfo img").attr('src','images/sign_up2.png');
            $("#oneInfo input").val("主类名字不能为空").css("color","#FF0000");
        }
    }
    function addC(){
        var url = "${pageContext.request.contextPath}/addJdTwoclass";
                ...
                if(0<val.length&&val.length<=5){
                    $.getJSON(url,args,function (date) {
                        if(date.name=="0"){
                            ...
                            $("#twoInfo input").show();
                            $("#twoInfo img").attr('src','images/sign_up2.png');
                            $("#twoInfo input").val("主类名字已存在").css("color","#FF0000");
                        }else{
//                alert(date.name)
//                alert(date.mid)
                            $("#twoInfo").show();
                            $("#twoInfo input").show();
                            $("#twoInfo img").attr('src','images/sign_up3.png');
                            $("#twoInfo input").val("增加成功").css("color","#1db31d");
                        }
                    })
                }else{
                    $("#twoInfo").show();
                ...
                $("#twoInfo input").show();
                $("#twoInfo img").attr('src','images/sign_up2.png');
                $("#twoInfo input").val("子类名字不能为空").css("color","#FF0000");
            }
        }else {
            $("#twoInfo").show();
            $("#twoInfo input").show();
            $("#twoInfo img").attr('src','images/sign_up2.png');
            $("#twoInfo input").val("请选择你要添加到的主类").css("color","#FF0000");
        }
    }
    function addCC(){
        var url = "${pageContext.request.contextPath}/addJdThreeclass";
                ...
                if(0<val.length&&val.length<=5){
                    $.getJSON(url,args,function (date) {
                        if(date.name=="0"){
                            ...
                            $("#twonInfo input").show();
                            $("#twonInfo img").attr('src','images/sign_up2.png');
                            $("#twonInfo input").val("子子分类名字名字已存在").css("color","#FF0000");
                        }else{
//                alert(date.name)
//                alert(date.mid)
                            $("#twonInfo").show();
                            $("#twonInfo input").show();
                            $("#twonInfo img").attr('src','images/sign_up3.png');
                            $("#twonInfo input").val("增加成功").css("color","#1db31d");
                        }
                    })
                }else{
                    $("#twonInfo").show();
                    $("#twonInfo input").show();
                    $("#twonInfo img").attr('src','images/sign_up2.png');
                    $("#twonInfo input").val("子子分类名字格式有误").css("color","#FF0000");
                }}else{
                $("#twonInfo").show();
                $("#twonInfo input").show();
                $("#twonInfo img").attr('src','images/sign_up2.png');
                $("#twonInfo input").val("子子分类名字不能为空").css("color","#FF0000");
            }
        }else {
            $("#twonInfo").show();
            $("#twonInfo input").show();
            $("#twonInfo img").attr('src','images/sign_up2.png');
            $("#twonInfo input").val("请选择你要添加到的子类").css("color","#FF0000");
        }
    }
    function showT(x,v) {
        var bool = $(v).attr("id");
...
//        alert(bool)
//        alert(x)
//        alert($(v.options[v.options.selectedIndex]).attr("name"))
        ...
        $.getJSON(url,args,function (date) {
            if(date.name=="0"){
                ...
                $("#"+bool+"nInfo").show();
                $("#"+bool+"nInfo"+" input").show();
                $("#"+bool+"nInfo"+" img").attr('src','images/sign_up2.png');
                $("#"+bool+"nInfo"+" input").val("此主类没有子类").css("color","#FF0000");
            }else{

                $("#"+bool+"nInfo").hide();
                $("#"+bool+"nInfo"+" input").hide();
    ...
    function showTh(x,v) {
        ...
        $.getJSON(url,args,function (date) {
            if(date.name=="0"){
                ...
                $("#threenInfo input").show();
                $("#threenInfo img").attr('src','images/sign_up2.png');
                $("#threenInfo input").val("此子类没有子子类").css("color","#FF0000");
            }else{
                $("#threenInfo").hide();
                $("#threenInfo input").hide();
                $("#Thone").empty();//清空info内容
                $("#Thone").append("" +
                    "<option name='' disabled='disabled' selected = 'selected'>选择你要增加到的子子类：</option>")
                $.each(date,function(i, item) {
    ...
    <nav class="navbar-default navbar-side" role="navigation">
        <div class="sidebar-collapse">
            <ul class="nav" id="main-menu">
                ...
                <li  >
                    <a  href="/Jd/s_table"><i class="fa fa-table fa-3x"></i>订单明细</a>
                </li>
                <li  >
                        ...
                        <div class="col-md-4">
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    ...
                                    <center>
                                        <a href="javascript:void(0);" onclick="addM()" class="btn btn-info">确定</a>
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    <div class="form-group" >
                                        ...
                                        <center>
                                            <a href="javascript:void(0);" onclick="addC()" class="btn btn-info">确定</a>
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    <div class="form-group" >
                                        ...
                                        <center>
                                            <a href="javascript:void(0);" onclick="addCC()" class="btn btn-info">确定</a>
                                        <p class="help-block" id="twonInfo" style="display:none">
                                            <img src="images/sign_up1.png" width="18" height="18">
                                            <input type="text" readonly style="background:transparent;border:0;">
                                        </p>
                                        </center>
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    <div class="form-group" >
                                        ...
                                        <br>
                                        <center>
                                            <a href="javascript:void(0);" onclick="addP()" class="btn btn-info">增加商品</a>
                                        <p class="help-block" id="threenInfo" style="display:none">
                                            <img src="images/sign_up1.png" width="18" height="18">
                                            <input type="text" readonly style="background:transparent;border:0;">
                                        </p>
                                        </center>
